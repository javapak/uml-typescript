/**
 * Implementation class for ReadSelfAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends Action
...
export class ReadSelfAction extends Action implements IReadSelfAction {
  /**
   * result
   * 
   * @type OutputPin
  ...
  constructor(init?: Partial<IReadSelfAction>) {
    super(init);
   ...
   */
  clone(): ReadSelfAction {
    return new ReadSelfAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IReadSelfAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ReadSelfAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ReadSelfAction{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): ReadSelfAction {
    const instance = new ReadSelfAction();
  ...
  static validate(instance: IReadSelfAction): ValidationResult {
    if (instance instanceof ReadSelfAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ReadSelfAction(instance);
