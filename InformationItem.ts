/**
 * Implementation class for InformationItem
 * 
 * @since UML 2.5
 * @package uml
 * @extends Classifier
...
export class InformationItem extends Classifier implements IInformationItem {
  /**
   * represented
   * 
   * @type Classifier
   ...
   */
  toJSON(): IInformationItem {
    const parentJson = super.toJSON();
    return {
      ...parentJson,
      represented: this.represented,
    };
  }
  ...
  toJSONSerialized(): any {
    ...
    result.represented = Array.from(this.represented);
   ...
   */
  update(values: Partial<IInformationItem>): void {
   ...
   */
  toString(): string {
    return `InformationItem{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): InformationItem {
    ...
    if (json.represented && Array.isArray(json.represented)) {
      instance.represented = new Set(json.represented);
    }
  ...
  static validate(instance: IInformationItem): ValidationResult {
    if (instance instanceof InformationItem) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new InformationItem(instance);
