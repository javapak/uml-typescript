/**
 * Implementation class for LoopNode
 * 
 * @since UML 2.5
 * @package uml
...
export class LoopNode extends StructuredActivityNode implements ILoopNode {
   ...
   */
  public loopVariable: IOutputPin[] = [];
   ...
   * @relationship containment
   */
  public loopVariableInput: IInputPin[] = [];
  ...
  getIsTestedFirst(): boolean {
    return this.isTestedFirst;
  }

  setIsTestedFirst(value: boolean): void {
    this.isTestedFirst = value;
  }

  getLoopVariable(): IOutputPin[] {
    return this.loopVariable;
  }

  setLoopVariable(value: IOutputPin[]): void {
    this.loopVariable = value;
  }

  getLoopVariableInput(): IInputPin[] {
    return this.loopVariableInput;
  }

  setLoopVariableInput(value: IInputPin[]): void {
    this.loopVariableInput = value;
  }
  ...
  toJSON(): ILoopNode {
    ...
    return {
      ...
      isTestedFirst: this.isTestedFirst,
      loopVariable: this.loopVariable,
      loopVariableInput: this.loopVariableInput,
      result: this.result,
      setupPart: this.setupPart,
      test: this.test,
    };
  }
  ...
  toJSONSerialized(): any {
    ...
    result.bodyPart = Array.from(this.bodyPart);
    result.decider = this.decider;
    result.isTestedFirst = this.isTestedFirst;
    result.loopVariable = [...this.loopVariable];
    result.loopVariableInput = [...this.loopVariableInput];
    result.result = [...this.result];
    result.setupPart = Array.from(this.setupPart);
   ...
   */
  update(values: Partial<ILoopNode>): void {
   ...
   */
  toString(): string {
    return `LoopNode{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): LoopNode {
    ...
    if (json.bodyPart && Array.isArray(json.bodyPart)) {
      instance.bodyPart = new Set(json.bodyPart);
    }
    instance.decider = json.decider;
    instance.isTestedFirst = json.isTestedFirst;
    if (json.loopVariable && Array.isArray(json.loopVariable)) {
      instance.loopVariable = [...json.loopVariable];
    }
    if (json.loopVariableInput && Array.isArray(json.loopVariableInput)) {
      instance.loopVariableInput = [...json.loopVariableInput];
    }
    if (json.result && Array.isArray(json.result)) {
      instance.result = [...json.result];
    }
    if (json.setupPart && Array.isArray(json.setupPart)) {
      instance.setupPart = new Set(json.setupPart);
    }
  ...
  validate(): ValidationResult {
    ...
    if (this.isTestedFirst == null) {
      errors.push({
        property: 'isTestedFirst',
        message: 'isTestedFirst is required',
        code: 'REQUIRED'
      });
    }
    if (this.isTestedFirst !== undefined) {
      if (typeof this.isTestedFirst !== 'boolean') {
        errors.push({
          property: 'isTestedFirst',
          message: 'isTestedFirst must be a boolean',
          code: 'TYPE_ERROR',
          value: this.isTestedFirst
        });
      }
    }
    if (this.loopVariable) {
    }
    if (this.loopVariableInput) {
    }
    if (this.result) {
    }
    if (this.setupPart) {
  ...
  static validate(instance: ILoopNode): ValidationResult {
    if (instance instanceof LoopNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new LoopNode(instance);
