/**
 * Implementation class for ReadLinkObjectEndAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends Action
...
export class ReadLinkObjectEndAction extends Action implements IReadLinkObjectEndAction {
  /**
   * end
   * 
   * @type Property
  ...
  constructor(init?: Partial<IReadLinkObjectEndAction>) {
    super(init);
    this.end = init?.end!;
   ...
   */
  toJSON(): IReadLinkObjectEndAction {
   ...
   */
  clone(): ReadLinkObjectEndAction {
    return new ReadLinkObjectEndAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IReadLinkObjectEndAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ReadLinkObjectEndAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ReadLinkObjectEndAction{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): ReadLinkObjectEndAction {
    const instance = new ReadLinkObjectEndAction();
  ...
  static validate(instance: IReadLinkObjectEndAction): ValidationResult {
    if (instance instanceof ReadLinkObjectEndAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ReadLinkObjectEndAction(instance);
