/**
 * Implementation class for FinalNode
 * 
 * @since UML 2.5
 * @package uml
 * @abstract
...
export class FinalNode extends ControlNode implements IFinalNode {
  ...
  constructor(init?: Partial<IFinalNode>) {
    super(init);
  }
  /**
   ...
   */
  update(values: Partial<IFinalNode>): void {
   ...
   */
  toString(): string {
    return `FinalNode{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): FinalNode {
    const instance = new FinalNode();
  ...
  static validate(instance: IFinalNode): ValidationResult {
    if (instance instanceof FinalNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new FinalNode(instance);
