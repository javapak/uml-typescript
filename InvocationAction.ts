/**
 * Implementation class for InvocationAction
 * 
 * @since UML 2.5
 * @package uml
 * @abstract
...
export class InvocationAction extends Action implements IInvocationAction {
  /**
   * argument
   * 
   * @type InputPin
  ...
  setArgument(value: IInputPin[]): void {
    this.argument = value;
  }
   ...
   */
  toJSON(): IInvocationAction {
    const parentJson = super.toJSON();
    return {
      ...parentJson,
      argument: this.argument,
      ...(this.onPort !== undefined && { onPort: this.onPort }),
    };
  }
  ...
  toJSONSerialized(): any {
    ...
    if (this.onPort !== undefined) {
      result.onPort = this.onPort;
    }
  ...
  clone(): InvocationAction {
    return new InvocationAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IInvocationAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof InvocationAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `InvocationAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IInvocationAction): ValidationResult {
    if (instance instanceof InvocationAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new InvocationAction(instance);
