/**
 * Implementation class for SequenceNode
 * 
 * @since UML 2.5
 * @package uml
...
export class SequenceNode extends StructuredActivityNode implements ISequenceNode {
  ...
  setExecutableNode(value: IExecutableNode[]): void {
    this.executableNode = value;
  }

  /**
   * Converts this instance to a plain object matching the interface
   */
  toJSON(): ISequenceNode {
    const parentJson = super.toJSON();
    return {
      ...parentJson,
      executableNode: this.executableNode,
    };
  }
  ...
  toJSONSerialized(): any {
    ...
    result.executableNode = [...this.executableNode];
   ...
   */
  update(values: Partial<ISequenceNode>): void {
   ...
   */
  toString(): string {
    return `SequenceNode{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): SequenceNode {
    ...
    if (json.executableNode && Array.isArray(json.executableNode)) {
      instance.executableNode = [...json.executableNode];
    }
  ...
  static validate(instance: ISequenceNode): ValidationResult {
    if (instance instanceof SequenceNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new SequenceNode(instance);
