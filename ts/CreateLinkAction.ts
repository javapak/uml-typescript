/**
 * Implementation class for CreateLinkAction
 * 
 * @since UML 2.5
 * @package uml
...
export class CreateLinkAction extends WriteLinkAction implements ICreateLinkAction {
   ...
   */
  clone(): CreateLinkAction {
    return new CreateLinkAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<ICreateLinkAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof CreateLinkAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `CreateLinkAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ICreateLinkAction): ValidationResult {
    if (instance instanceof CreateLinkAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new CreateLinkAction(instance);
