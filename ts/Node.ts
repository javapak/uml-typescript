...
export class Node extends Class implements INode {
  ...
  setNestedNode(value: Set<INode>): void {
    this.nestedNode = value;
  }
  ...
  toJSONSerialized(): any {
    ...
    result.nestedNode = Array.from(this.nestedNode);
   ...
   */
  update(values: Partial<INode>): void {
   ...
   */
  toString(): string {
    return `Node{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: INode): ValidationResult {
    if (instance instanceof Node) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new Node(instance);
