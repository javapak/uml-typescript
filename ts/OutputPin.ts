/**
 * Implementation class for OutputPin
 * 
 * @since UML 2.5
 * @package uml
 * @extends Pin
...
export class OutputPin extends Pin implements IOutputPin {
  /**
   * ownedComment
   * 
   * @type Comment
  ...
  constructor(init?: Partial<IOutputPin>) {
    super(init);
   ...
   */
  update(values: Partial<IOutputPin>): void {
   ...
   */
  toString(): string {
    return `OutputPin{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): OutputPin {
    const instance = new OutputPin();
  ...
  static validate(instance: IOutputPin): ValidationResult {
    if (instance instanceof OutputPin) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new OutputPin(instance);
