/**
 * Implementation class for Actor
 * 
 * @since UML 2.5
 * @package uml
...
export class Actor extends BehavioredClassifier implements IActor {
   ...
   */
  update(values: Partial<IActor>): void {
   ...
   */
  toString(): string {
    return `Actor{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IActor): ValidationResult {
    if (instance instanceof Actor) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new Actor(instance);
