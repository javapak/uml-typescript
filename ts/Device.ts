/**
 * Implementation class for Device
 * 
 * @since UML 2.5
 * @package uml
 * @extends Node
...
export class Device extends Node implements IDevice {
   ...
   */
  update(values: Partial<IDevice>): void {
   ...
   */
  toString(): string {
    return `Device{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IDevice): ValidationResult {
    if (instance instanceof Device) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new Device(instance);
