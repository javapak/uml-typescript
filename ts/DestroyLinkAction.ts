/**
 * Implementation class for DestroyLinkAction
 * 
 * @since UML 2.5
 * @package uml
...
export class DestroyLinkAction extends WriteLinkAction implements IDestroyLinkAction {
  /**
   * ownedComment
   * 
   * @type Comment
  ...
  constructor(init?: Partial<IDestroyLinkAction>) {
    super(init);
   ...
   */
  clone(): DestroyLinkAction {
    return new DestroyLinkAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IDestroyLinkAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof DestroyLinkAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `DestroyLinkAction{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): DestroyLinkAction {
    const instance = new DestroyLinkAction();
  ...
  static validate(instance: IDestroyLinkAction): ValidationResult {
    if (instance instanceof DestroyLinkAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new DestroyLinkAction(instance);
