/**
 * Implementation class for RaiseExceptionAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends Action
...
export class RaiseExceptionAction extends Action implements IRaiseExceptionAction {
  /**
   * exception
   * 
   * @type InputPin
  ...
  constructor(init?: Partial<IRaiseExceptionAction>) {
    super(init);
    this.exception = init?.exception!;
  }
  getException(): IInputPin {
    return this.exception;
  }

  setException(value: IInputPin): void {
   ...
   */
  clone(): RaiseExceptionAction {
    return new RaiseExceptionAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IRaiseExceptionAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof RaiseExceptionAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `RaiseExceptionAction{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): RaiseExceptionAction {
    const instance = new RaiseExceptionAction();
    ...
    instance.exception = json.exception;
  ...
  static validate(instance: IRaiseExceptionAction): ValidationResult {
    if (instance instanceof RaiseExceptionAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new RaiseExceptionAction(instance);
