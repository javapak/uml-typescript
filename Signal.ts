/**
 * Implementation class for Signal
 * 
 * @since UML 2.5
 * @package uml
 * @extends Classifier
...
export class Signal extends Classifier implements ISignal {
  /**
   * ownedAttribute
   * 
   * @type Property
  ...
  constructor(init?: Partial<ISignal>) {
    super(init);
   ...
   */
  update(values: Partial<ISignal>): void {
   ...
   */
  toString(): string {
    return `Signal{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ISignal): ValidationResult {
    if (instance instanceof Signal) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new Signal(instance);
