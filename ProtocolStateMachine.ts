/**
 * Implementation class for ProtocolStateMachine
 * 
 * @since UML 2.5
 * @package uml
...
export class ProtocolStateMachine extends StateMachine implements IProtocolStateMachine {
  /**
   * conformance
   * 
  ...
  setConformance(value: Set<IProtocolConformance>): void {
  ...
  toJSONSerialized(): any {
    ...
    result.conformance = Array.from(this.conformance);
   ...
   */
  clone(): ProtocolStateMachine {
    return new ProtocolStateMachine(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IProtocolStateMachine>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ProtocolStateMachine)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ProtocolStateMachine{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): ProtocolStateMachine {
    ...
    if (json.submachineState && Array.isArray(json.submachineState)) {
      instance.submachineState = new Set(json.submachineState);
    }
    if (json.region && Array.isArray(json.region)) {
      instance.region = new Set(json.region);
    }
    if (json.extendedStateMachine && Array.isArray(json.extendedStateMachine)) {
      instance.extendedStateMachine = new Set(json.extendedStateMachine);
    }
    if (json.conformance && Array.isArray(json.conformance)) {
      instance.conformance = new Set(json.conformance);
    }
  ...
  static validate(instance: IProtocolStateMachine): ValidationResult {
    if (instance instanceof ProtocolStateMachine) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ProtocolStateMachine(instance);
