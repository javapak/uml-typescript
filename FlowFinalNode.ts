/**
 * Implementation class for FlowFinalNode
 * 
 * @since UML 2.5
 * @package uml
 * @extends FinalNode
...
export class FlowFinalNode extends FinalNode implements IFlowFinalNode {
  /**
   * ownedComment
   * 
   * @type Comment
  ...
  constructor(init?: Partial<IFlowFinalNode>) {
    super(init);
   ...
   */
  clone(): FlowFinalNode {
    return new FlowFinalNode(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IFlowFinalNode>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof FlowFinalNode)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `FlowFinalNode{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): FlowFinalNode {
    const instance = new FlowFinalNode();
  ...
  static validate(instance: IFlowFinalNode): ValidationResult {
    if (instance instanceof FlowFinalNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new FlowFinalNode(instance);
