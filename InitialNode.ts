/**
 * Implementation class for InitialNode
 * 
 * @since UML 2.5
 * @package uml
 * @extends ControlNode
...
export class InitialNode extends ControlNode implements IInitialNode {
  ...
  constructor(init?: Partial<IInitialNode>) {
    super(init);
  }
  /**
   ...
   */
  clone(): InitialNode {
    return new InitialNode(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IInitialNode>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof InitialNode)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `InitialNode{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): InitialNode {
    const instance = new InitialNode();
  ...
  static validate(instance: IInitialNode): ValidationResult {
    if (instance instanceof InitialNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new InitialNode(instance);
