/**
 * Implementation class for Stereotype
 * 
 * @since UML 2.5
 * @package uml
 * @extends Class
...
export class Stereotype extends Class implements IStereotype {
  /**
   * icon
   * 
   * @type Image
  ...
  toJSONSerialized(): any {
    ...
    result.icon = Array.from(this.icon);
   ...
   */
  clone(): Stereotype {
    return new Stereotype(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IStereotype>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof Stereotype)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `Stereotype{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): Stereotype {
    ...
    if (json.icon && Array.isArray(json.icon)) {
      instance.icon = new Set(json.icon);
    }
  ...
  static validate(instance: IStereotype): ValidationResult {
    if (instance instanceof Stereotype) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new Stereotype(instance);
