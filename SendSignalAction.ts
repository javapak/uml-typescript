/**
 * Implementation class for SendSignalAction
 * 
 * @since UML 2.5
 * @package uml
...
export class SendSignalAction extends InvocationAction implements ISendSignalAction {
  /**
   * signal
   * 
   * @type Signal
   ...
   */
  update(values: Partial<ISendSignalAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof SendSignalAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `SendSignalAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ISendSignalAction): ValidationResult {
    if (instance instanceof SendSignalAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new SendSignalAction(instance);
