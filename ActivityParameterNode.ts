/**
 * Implementation class for ActivityParameterNode
 * 
 * @since UML 2.5
 * @package uml
 * @extends ObjectNode
...
export class ActivityParameterNode extends ObjectNode implements IActivityParameterNode {
  /**
   * parameter
   * 
   * @type Parameter
   ...
   */
  clone(): ActivityParameterNode {
    return new ActivityParameterNode(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IActivityParameterNode>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ActivityParameterNode)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ActivityParameterNode{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IActivityParameterNode): ValidationResult {
    if (instance instanceof ActivityParameterNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ActivityParameterNode(instance);
