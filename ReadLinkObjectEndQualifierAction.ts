/**
 * Implementation class for ReadLinkObjectEndQualifierAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends Action
...
export class ReadLinkObjectEndQualifierAction extends Action implements IReadLinkObjectEndQualifierAction {
  /**
   * object
   * 
   * @type InputPin
   ...
   */
  toJSON(): IReadLinkObjectEndQualifierAction {
   ...
   */
  clone(): ReadLinkObjectEndQualifierAction {
    return new ReadLinkObjectEndQualifierAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IReadLinkObjectEndQualifierAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ReadLinkObjectEndQualifierAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ReadLinkObjectEndQualifierAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IReadLinkObjectEndQualifierAction): ValidationResult {
    if (instance instanceof ReadLinkObjectEndQualifierAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ReadLinkObjectEndQualifierAction(instance);
