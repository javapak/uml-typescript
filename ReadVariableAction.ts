/**
 * Implementation class for ReadVariableAction
 * 
 * @since UML 2.5
 * @package uml
...
export class ReadVariableAction extends VariableAction implements IReadVariableAction {
  /**
   * result
   * 
   * @type OutputPin
   ...
   */
  clone(): ReadVariableAction {
    return new ReadVariableAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IReadVariableAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ReadVariableAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ReadVariableAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IReadVariableAction): ValidationResult {
    if (instance instanceof ReadVariableAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ReadVariableAction(instance);
