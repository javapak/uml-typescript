/**
 * Implementation class for CentralBufferNode
 * 
 * @since UML 2.5
 * @package uml
 * @extends ObjectNode
...
export class CentralBufferNode extends ObjectNode implements ICentralBufferNode {
   ...
   */
  clone(): CentralBufferNode {
    return new CentralBufferNode(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<ICentralBufferNode>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof CentralBufferNode)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `CentralBufferNode{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ICentralBufferNode): ValidationResult {
    if (instance instanceof CentralBufferNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new CentralBufferNode(instance);
