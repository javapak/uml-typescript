/**
 * Implementation class for PrimitiveType
 * 
 * @since UML 2.5
 * @package uml
 * @extends DataType
...
export class PrimitiveType extends DataType implements IPrimitiveType {
   ...
   */
  update(values: Partial<IPrimitiveType>): void {
   ...
   */
  toString(): string {
    return `PrimitiveType{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IPrimitiveType): ValidationResult {
    if (instance instanceof PrimitiveType) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new PrimitiveType(instance);
