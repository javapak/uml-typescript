/**
 * Implementation class for StateMachine
 * 
 * @since UML 2.5
 * @package uml
 * @extends Behavior
...
export class StateMachine extends Behavior implements IStateMachine {
  ...
  toJSON(): IStateMachine {
    ...
    return {
      ...
      submachineState: this.submachineState,
      region: this.region,
      extendedStateMachine: this.extendedStateMachine,
    };
  }
  ...
  toJSONSerialized(): any {
    ...
    result.submachineState = Array.from(this.submachineState);
    result.region = Array.from(this.region);
    result.extendedStateMachine = Array.from(this.extendedStateMachine);
   ...
   */
  update(values: Partial<IStateMachine>): void {
   ...
   */
  toString(): string {
    return `StateMachine{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IStateMachine): ValidationResult {
    if (instance instanceof StateMachine) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new StateMachine(instance);
