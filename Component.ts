...
export class Component extends Class implements IComponent {
  /**
   ...
   * @multiplicity [1..1]
   */
  public isIndirectlyInstantiated!: boolean;
  ...
  setIsIndirectlyInstantiated(value: boolean): void {
    this.isIndirectlyInstantiated = value;
  }
  ...
  getRealization(): Set<IComponentRealization> {
    return this.realization;
  }

  setRealization(value: Set<IComponentRealization>): void {
  ...
  toJSON(): IComponent {
    ...
    return {
      ...parentJson,
      isIndirectlyInstantiated: this.isIndirectlyInstantiated,
  ...
  toJSONSerialized(): any {
    ...
    result.isIndirectlyInstantiated = this.isIndirectlyInstantiated;
   ...
   */
  update(values: Partial<IComponent>): void {
   ...
   */
  toString(): string {
    return `Component{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): Component {
    ...
    instance.isIndirectlyInstantiated = json.isIndirectlyInstantiated;
  ...
  validate(): ValidationResult {
    ...
    if (this.isIndirectlyInstantiated == null) {
      errors.push({
        property: 'isIndirectlyInstantiated',
        message: 'isIndirectlyInstantiated is required',
        code: 'REQUIRED'
      });
    }
    if (this.isIndirectlyInstantiated !== undefined) {
      if (typeof this.isIndirectlyInstantiated !== 'boolean') {
        errors.push({
          property: 'isIndirectlyInstantiated',
          message: 'isIndirectlyInstantiated must be a boolean',
          code: 'TYPE_ERROR',
          value: this.isIndirectlyInstantiated
        });
      }
    }
  ...
  static validate(instance: IComponent): ValidationResult {
    if (instance instanceof Component) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new Component(instance);
