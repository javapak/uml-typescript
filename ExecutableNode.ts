/**
 * Implementation class for ExecutableNode
 * 
 * @since UML 2.5
 * @package uml
 * @abstract
...
export class ExecutableNode extends ActivityNode implements IExecutableNode {
  /**
   * handler
   * 
  ...
  constructor(init?: Partial<IExecutableNode>) {
    super(init);

    this.handler = init?.handler ? new Set(init.handler) : new Set();
  }
  getHandler(): Set<IExceptionHandler> {
    return this.handler;
  }

  setHandler(value: Set<IExceptionHandler>): void {
    this.handler = value;
  }
  ...
  toJSONSerialized(): any {
    ...
    result.handler = Array.from(this.handler);
   ...
   */
  update(values: Partial<IExecutableNode>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ExecutableNode)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ExecutableNode{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): ExecutableNode {
    const instance = new ExecutableNode();
  ...
  static validate(instance: IExecutableNode): ValidationResult {
    if (instance instanceof ExecutableNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ExecutableNode(instance);
