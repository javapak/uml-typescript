/**
 * Implementation class for ReadIsClassifiedObjectAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends Action
...
export class ReadIsClassifiedObjectAction extends Action implements IReadIsClassifiedObjectAction {
  /**
   * classifier
   * 
   * @type Classifier
   ...
   */
  toJSON(): IReadIsClassifiedObjectAction {
   ...
   */
  clone(): ReadIsClassifiedObjectAction {
    return new ReadIsClassifiedObjectAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IReadIsClassifiedObjectAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ReadIsClassifiedObjectAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ReadIsClassifiedObjectAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static fromJSON(json: any): ReadIsClassifiedObjectAction {
    ...
    instance.isDirect = json.isDirect;
  ...
  validate(): ValidationResult {
    ...
    if (this.isDirect == null) {
      errors.push({
        property: 'isDirect',
        message: 'isDirect is required',
        code: 'REQUIRED'
      });
    }
    if (this.isDirect !== undefined) {
      if (typeof this.isDirect !== 'boolean') {
        errors.push({
          property: 'isDirect',
          message: 'isDirect must be a boolean',
          code: 'TYPE_ERROR',
          value: this.isDirect
        });
      }
  ...
  static validate(instance: IReadIsClassifiedObjectAction): ValidationResult {
    if (instance instanceof ReadIsClassifiedObjectAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ReadIsClassifiedObjectAction(instance);
