/**
 * Implementation class for ForkNode
 * 
 * @since UML 2.5
 * @package uml
 * @extends ControlNode
...
export class ForkNode extends ControlNode implements IForkNode {
  /**
   * ownedComment
   * 
   * @type Comment
  ...
  constructor(init?: Partial<IForkNode>) {
    super(init);
   ...
   */
  update(values: Partial<IForkNode>): void {
   ...
   */
  toString(): string {
    return `ForkNode{name: ${this.name || 'unnamed'}}`;
  }

  /**
   * Creates an instance from a JSON-serialized object
   */
  static fromJSON(json: any): ForkNode {
    const instance = new ForkNode();
  ...
  static validate(instance: IForkNode): ValidationResult {
    if (instance instanceof ForkNode) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ForkNode(instance);
