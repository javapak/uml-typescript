/**
 * Implementation class for CreateObjectAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends Action
...
export class CreateObjectAction extends Action implements ICreateObjectAction {
  /**
   * classifier
   * 
   * @type Classifier
   ...
   */
  clone(): CreateObjectAction {
    return new CreateObjectAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<ICreateObjectAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof CreateObjectAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `CreateObjectAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ICreateObjectAction): ValidationResult {
    if (instance instanceof CreateObjectAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new CreateObjectAction(instance);
