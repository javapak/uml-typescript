/**
 * Implementation class for SendObjectAction
 * 
 * @since UML 2.5
 * @package uml
...
export class SendObjectAction extends InvocationAction implements ISendObjectAction {
  /**
   * request
   * 
   * @type InputPin
  ...
  getRequest(): IInputPin {
    return this.request;
  }

  setRequest(value: IInputPin): void {
    this.request = value;
  }
   ...
   */
  clone(): SendObjectAction {
    return new SendObjectAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<ISendObjectAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof SendObjectAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `SendObjectAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ISendObjectAction): ValidationResult {
    if (instance instanceof SendObjectAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new SendObjectAction(instance);
