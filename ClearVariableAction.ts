/**
 * Implementation class for ClearVariableAction
 * 
 * @since UML 2.5
 * @package uml
...
export class ClearVariableAction extends VariableAction implements IClearVariableAction {
   ...
   */
  clone(): ClearVariableAction {
    return new ClearVariableAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<IClearVariableAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof ClearVariableAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `ClearVariableAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: IClearVariableAction): ValidationResult {
    if (instance instanceof ClearVariableAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new ClearVariableAction(instance);
