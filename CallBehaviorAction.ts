/**
 * Implementation class for CallBehaviorAction
 * 
 * @since UML 2.5
 * @package uml
 * @extends CallAction
...
export class CallBehaviorAction extends CallAction implements ICallBehaviorAction {
  /**
   * behavior
   * 
   * @type Behavior
   ...
   */
  clone(): CallBehaviorAction {
    return new CallBehaviorAction(this);
  }

  /**
   * Updates this instance with values from another object
   */
  update(values: Partial<ICallBehaviorAction>): void {
  ...
  equals(other: any): boolean {
    if (!(other instanceof CallBehaviorAction)) {
      return false;
    }
   ...
   */
  toString(): string {
    return `CallBehaviorAction{name: ${this.name || 'unnamed'}}`;
  }
  ...
  static validate(instance: ICallBehaviorAction): ValidationResult {
    if (instance instanceof CallBehaviorAction) {
      return instance.validate();
    }
    // Validate plain object
    const temp = new CallBehaviorAction(instance);
